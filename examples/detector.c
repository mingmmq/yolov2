...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    box *boxes = calloc(l.w*l.h*l.n, sizeof(box));
    float **probs = calloc(l.w*l.h*l.n, sizeof(float *));
    if (debug) {fprintf(stderr, "[validate_detector] l.w:%d, l.h:%d, l.h:%d, [probs_size]:%d\r\n", l.w, l.h, l.n, l.w*l.h*l.n);}
    for(j = 0; j < l.w*l.h*l.n; ++j) probs[j] = calloc(classes+1, sizeof(float *));
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            int w = val[t].w;
            int h = val[t].h;
            //first the region boxes
            get_region_boxes(l, w, h, net->w, net->h, thresh, probs, boxes, 0, 0, map, .5, 0);
//            if (debug) {fprintf(stderr, "[validate_detector] classes:%d\r\n", classes);}
            if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, classes, nms);
            if (coco){
                print_cocos(fp, path, boxes, probs, l.w*l.h*l.n, classes, w, h);
            } else if (imagenet){
...
void validate_detector_without_nms(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.list");
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            //first the region boxes
            get_region_boxes(l, w, h, net->w, net->h, thresh, probs, boxes, 0, 0, map, .5, 0);
            //todo1: not doing the nms sort, keep all the predicitions, and use the number of the ground
            //truth to get the final prediction
//            if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, classes, nms);
            if (coco){
                print_cocos(fp, path, boxes, probs, l.w*l.h*l.n, classes, w, h);
            } else if (imagenet){
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "nonms")) validate_detector_without_nms(datacfg, cfg, weights, outfile);
